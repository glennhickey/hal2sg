...
CREATE TABLE Sequence (ID INTEGER PRIMARY KEY,
	...
	md5checksum TEXT NOT NULL, -- Shouldn't this be in the Sequence table instead?
	length INTEGER NOT NULL,
	FOREIGN KEY(fastaID) REFERENCES FASTA(ID));
CREATE TABLE GraphJoin (ID INTEGER PRIMARY KEY,
	-- startJoin defines parent sequence & position that 5' end attaches to. May be null
	side1SequenceID INTEGER, side1Position INTEGER, side1StrandIsForward BOOLEAN, 
	-- endJoin defines parent sequence & position that 3' end attaches to. May be null
	side2SequenceID INTEGER, side2Position INTEGER, side2StrandIsForward BOOLEAN, 
...
CREATE TABLE Reference (ID INTEGER PRIMARY KEY, 
	...
	sourceDivergence TEXT, -- may be null if isDerived is FALSE (not sure if it needs to be stored)?
